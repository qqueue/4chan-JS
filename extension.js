...
Parser.init = function() {
  ...
  this.customSpoiler = {};
  if (Config.localTime) {
    this.hasSeconds = (el = $.cls('dateTime')[1]) && /\d{2}:\d{2}:\d{2}$/.test(el.textContent);
...
Parser.saveTrackedReplies = function(tid, replies) {
  sessionStorage.setItem('4chan-track-' + Main.board + '-' + tid, JSON.stringify(replies));
};
...
Parser.setCustomSpoiler = function(board, val) {
  ...
  if (!this.customSpoiler[board] && (val = parseInt(val))) {
    if (board == Main.board && (s = $.cls('imgspoiler')[0])) {
      this.customSpoiler[board] = s.firstChild.src.match(/spoiler(-[a-z0-9]+)\.png$/)[1];
    } else {
      this.customSpoiler[board] = '-' + board + (Math.floor(Math.random() * val) + 1);
    }
  }
};
Parser.buildPost = function(thread, board, pid) {
  ...
  for (i = 0; j = thread[i]; ++i) {
    ...
    if (Config.IDColor && (uid = $.cls('posteruid', el)[1])) {
...
Parser.buildHTMLFromJSON = function(data, board, standalone, fromQuote) {
  ...
  if (!data.capcode && data.id) {
    userId = ' <span class="posteruid id_' + data.id + '">(ID: <span class="hand" title="Highlight posts by this ID">' + data.id + '</span>)</span> ';
  } else {
    userId = '';
  }
  switch (data.capcode) {
    ...
    case 'mod':
      ...
      capcode = ' <img src="' + Parser.icons.mod + '" ' + 'alt="This user is a 4chan Moderator." ' + 'title="This user is a 4chan Moderator." class="identityIcon">';
      break;
    case 'developer':
      capcodeStart = ' <strong class="capcode hand id_developer" ' + 'title="Highlight posts by Developers">## Developer</strong>';
      capcodeClass = ' capcodeDeveloper';
      capcode = ' <img src="' + Parser.icons.dev + '" ' + 'alt="This user is a 4chan Developer." ' + 'title="This user is a 4chan Developer." class="identityIcon">';
      break;
    case 'manager':
  ...
  if (data.country) {
    if (board == 'pol') {
      flag = ' <img src="//s.4cdn.org/image/country/troll/' + data.country.toLowerCase() + '.gif" alt="' + data.country + '" title="' + data.country_name + '" class="countryFlag">';
    } else {
      flag = ' <span title="' + data.country_name + '" class="flag flag-' + data.country.toLowerCase() + '"></span>';
    }
  } else {
    flag = '';
  }
  if (data.filedeleted) {
    fileHtml = '<div id="f' + data.no + '" class="file"><span class="fileThumb"><img src="' + Parser.icons.del + '" class="fileDeletedRes" alt="File deleted."></span></div>';
  } else if (data.ext) {
    ...
    if (decodedFilename.length > (isOP ? 40 : 30)) {
      shortFile = Parser.encodeSpecialChars(decodedFilename.slice(0, isOP ? 35 : 25)) + '(...)' + data.ext;
      needFileTip = true;
    }
  ...
  if (isOP) {
    ...
    if (data.sticky) {
      threadIcons += '<img class="stickyIcon retina" title="Sticky" alt="Sticky" src="' + Main.icons2.sticky + '"> ';
    }
    if (data.closed) {
      threadIcons += '<img class="closedIcon retina" title="Closed" alt="Closed" src="' + Main.icons2.closed + '"> ';
    }
  }
  container.className = 'postContainer ' + postType + 'Container';
  container.id = 'pc' + data.no;
  container.innerHTML = (isOP ? '' : '<div class="sideArrows" id="sa' + data.no + '">&gt;&gt;</div>') + '<div id="p' + data.no + '" class="post ' + postType + highlight + '">' + '<div class="postInfoM mobile" id="pim' + data.no + '">' + '<span class="nameBlock' + capcodeClass + '">' + '<span class="name">' + name + '</span>' + tripcode +
    capcodeStart + capcode + userId + flag + '<br><span class="subject">' + subject + '</span></span><span class="dateTime postNum" data-utc="' + data.time + '">' +
    data.now + ' <a href="' + data.no + '#p' + data.no + '">No.</a>' + '<a href="javascript:quote(\'' + data.no + '\');" title="Quote this post">' +
    data.no + '</a></span>' + '</div>' +
    (isOP ? fileHtml : '') + '<div class="postInfo desktop" id="pi' + data.no + '">' + '<input type="checkbox" name="' + data.no + '" value="delete"> ' + '<span class="subject">' + subject + '</span> ' + '<span class="nameBlock' + capcodeClass + '">' + emailStart + '<span class="name">' + name + '</span>' +
    tripcode + capcodeStart + emailEnd + capcode + userId + flag + ' </span> ' + '<span class="dateTime" data-utc="' + data.time + '">' + data.now + '</span> ' + '<span class="postNum desktop">' + '<a href="' + noLink + '" title="Highlight this post">No.</a><a href="' +
    quoteLink + '" title="Quote this post">' + data.no + '</a> ' + threadIcons + replySpan + '</span>' + '</div>' +
    (isOP ? '' : fileHtml) + '<blockquote class="postMessage" id="m' + data.no + '">' + (data.com || '') + capcode_replies + summary + '</blockquote> ' + '</div>' + mobileLink;
  if (!Main.tid || board != Main.board) {
...
Parser.buildCapcodeReplies = function(replies, board, tid) {
  ...
  for (capcode in replies) {
    ...
    for (i = 0; id = post_ids[i]; ++i) {
      html += '<a class="quotelink" href="' + prelink + tid + '#p' + id + '">' + pretext + id + '</a> ';
    }
  }
...
Parser.parseThread = function(tid, offset, limit) {
  var i, j, thread, posts, pi, el, frag, summary, omitted, key, filtered, cnt, frag;
  thread = $.id('t' + tid);
  ...
  if (!offset) {
    ...
    if (!Main.tid) {
      if (Config.filter) {
        filtered = Filter.exec(thread, pi, document.getElementById('m' + tid), tid);
      }
      if (Config.threadHiding && !filtered) {
        if (Main.hasMobileLayout) {
          ...
          el.innerHTML = '<img alt="H" class="extButton threadHideButton"' + 'data-cmd="hide" data-id="' + tid + '" src="' + Main.icons.minus + '" title="Toggle thread">';
      ...
      if (ThreadExpansion.enabled && (summary = $.cls('summary', thread)[0])) {
...
Parser.parsePost = function(pid, tid) {
  ...
  if (Config.revealSpoilers && (file = document.getElementById('f' + pid)) && (file = file.children[1])) {
...
Parser.getLocaleDate = function(date) {
  return ('0' + (1 + date.getMonth())).slice(-2) + '/' + ('0' + date.getDate()).slice(-2) + '/' + ('0' + date.getFullYear()).slice(-2) + '(' + this.weekdays[date.getDay()] + ')' + ('0' + date.getHours()).slice(-2) + ':' + ('0' + date.getMinutes()).slice(-2) + (this.hasSeconds ? (':' + ('0' + date.getSeconds()).slice(-2)) : '');
};
...
Depager.init = function() {
  ...
  if (el = $.cls('prev')[0]) {
    el.innerHTML = '[<a title="Toggle infinite scroll" ' + 'class="depagelink" href="" data-cmd="depage">All</a>]';
...
Depager.onScroll = function() {
  if (document.documentElement.scrollHeight <= (window.innerHeight + window.pageYOffset + Depager.threshold)) {
...
Depager.renderNext = function() {
  var el, frag, i, j, k, threads, op, summary, cnt, reply, parseList, scroll, lastReplies, pageId, data, isLastPage, html;
  parseList = [];
...
QuoteInline.isSelfQuote = function(node, pid, board) {
  ...
  if ((node.nodeName == 'BLOCKQUOTE' && node.id.split('m')[1] == pid) || node.parentNode.id.split('_')[1] == pid) {
    return true;
  }
  return false;
};
...
QuotePreview.show = function(link, post, remote) {
  var rect, postHeight, postWidth, doc, docWidth, style, pos, quotes, i, j, qid, top, scrollTop, margin, img;
  if (remote) {
  ...
  if (!link.parentNode.className) {
    quotes = post.querySelectorAll('#' + $.cls('postMessage', post)[0].id + ' > .quotelink');
    if (quotes[1]) {
  ...
  if (Main.isMobileDevice) {
    ...
    top = rect.top + link.offsetHeight + window.pageYOffset - post.offsetHeight / 2 - rect.height / 2;
...
QR.quotePost = function(tid, pid) {
  if (!QR.noCooldown && (Main.threadClosed || (!Main.tid && Main.isThreadClosed(tid)))) {
...
QR.addQuote = function(pid) {
  ...
  if (ta.value) {
    ta.value = ta.value.slice(0, pos) + q + ta.value.slice(ta.selectionEnd);
  } else {
    ta.value = q;
  }
  if (UA.isOpera) {
...
QR.show = function(tid) {
  var i, j, cnt, postForm, form, qrForm, fields, row, spoiler, file, el, el2, placeholder, cd, qrError, cookie;
  if (QR.currentTid) {
  ...
  cnt.innerHTML = '<div id="qrHeader" class="drag postblock">Quick Reply - Thread No.<span id="qrTid">' + tid + '</span><img alt="X" src="' + Main.icons.cross + '" id="qrClose" ' + 'class="extButton" title="Close Window"></div>';
  form = postForm.parentNode.cloneNode(false);
  form.setAttribute('name', 'qrPost');
  form.innerHTML = '<input type="hidden" value="' + $.byName('MAX_FILE_SIZE')[0].value + '" name="MAX_FILE_SIZE">' + '<input type="hidden" value="regist" name="mode">' + '<input id="qrResto" type="hidden" value="' + tid + '" name="resto">';
    ...
    if (fields[i].id == 'captchaFormPart') {
      ...
      if (placeholder == 'Password' || placeholder == 'Spoilers') {
        ...
        if (el.nodeName == 'INPUT' || el.nodeName == 'TEXTAREA') {
          ...
          if ((cookie = Main.getCookie('4chan_flag')) && (el2 = el.querySelector('option[value="' + cookie + '"]'))) {
...
QR.checkComLength = function() {
  ...
  if (QR.comLength) {
    ...
    if (byteLength > QR.comLength) {
      QR.showPostError('Error: Comment too long (' + byteLength + '/' + QR.comLength + ').', 'length', true);
    } else {
...
QR.reloadCaptcha = function(focus) {
  var pulse, poll, el;
  if (QR.noCaptcha || !(el = $.id('recaptcha_image')) || !window.RecaptchaState) {
    return;
  }
...
QR.showPostError = function(msg, type, silent) {
  ...
  if (!silent && (document.hidden || document.mozHidden || document.webkitHidden || document.msHidden)) {
    alert('Posting Error');
  }
};
...
ThreadWatcher.init = function() {
  ...
  cnt.innerHTML = '<div class="drag" id="twHeader">Thread Watcher' + (UA.hasCORS ? ('<img id="twPrune" class="pointer right" src="' + Main.icons.refresh + '" alt="R" title="Refresh"></div>') : '</div>');
...
ThreadWatcher.build = function(rebuildButtons) {
  ...
  for (key in this.watched) {
    tuid = key.split('-');
    html += '<li id="watch-' + key + '"><span class="pointer" data-cmd="unwatch" data-id="' + tuid[0] + '" data-board="' + tuid[1] + '">&times;</span> <a href="' + Main.linkToThread(tuid[0], tuid[1]) + '#lr' + this.watched[key][1] + '"';
...
ThreadWatcher.onClick = function(e) {
  ...
  if (t.hasAttribute('data-id')) {
    ThreadWatcher.toggle(t.getAttribute('data-id'), t.getAttribute('data-board'));
...
ThreadWatcher.toggle = function(tid, board, synced) {
  ...
  if (this.watched[key]) {
    ...
    if (label = $.cls('subject', $.id('pi' + tid))[0].textContent) {
      ...
      label = label.replace(/(?:<br>)+/g, ' ').replace(/<[^>]*?>/g, '').slice(0, this.charLimit);
    } else {
      label = 'No.' + tid;
    }
  ...
  $.get('//a.4cdn.org/' + Main.board + '/res/' + tid + '.json', {
    onload: function() {
      ...
      if (this.status == 200) {
        ...
        msg = $.id('m' + tid);
        if ((abbr = $.cls('abbr', msg)[0]) && /^Comment/.test(abbr.textContent)) {
...
ThreadUpdater.onScroll = function(e) {
  if (ThreadUpdater.hadAuto && (document.documentElement.scrollHeight <= (window.innerHeight + window.pageYOffset) && !document[ThreadUpdater.hidden])) {
...
ThreadUpdater.onload = function() {
  ...
  if (this.status == 200) {
    ...
    if (count) {
      doc = document.documentElement;
      autoscroll = (Config.autoScroll && document[self.hidden] && doc.scrollHeight == (window.innerHeight + window.pageYOffset));
      frag = document.createDocumentFragment();
...
ThreadUpdater.setError = function(msg) {
  this.statusNode.innerHTML = this.statusNodeBot.innerHTML = '<span class="tu-error">' + msg + '</span>';
};
...
Filter.exec = function(cnt, pi, msg, tid) {
  ...
  for (i = 0; f = filters[i]; ++i) {
    if (f.type == 0) {
      if ((trip || (trip = pi.getElementsByClassName('postertrip')[0])) && f.pattern == trip.textContent) {
        hit = true;
        break;
      }
    } else if (f.type == 1) {
      if ((name || (name = pi.getElementsByClassName('name')[0])) && f.pattern == name.textContent) {
        hit = true;
        break;
      }
    } else if (f.type == 2) {
      if (!com) {
        this.entities.innerHTML = msg.innerHTML.replace(/<br>/g, '\n').replace(/[<[^>]+>/g, '');
      ...
      if ((mail || ((mail = pi.getElementsByClassName('useremail')[0]) && (mail = mail.href.slice(7)))) && f.pattern.test(mail)) {
        hit = true;
        break;
      }
    } else if (f.type == 4) {
      if ((uid || ((uid = pi.getElementsByClassName('posteruid')[0]) && (uid = uid.firstElementChild.textContent))) && f.pattern == uid) {
        hit = true;
        break;
      }
    } else {
      if ((sub || ((sub = pi.getElementsByClassName('subject')[0]) && (sub = sub.textContent))) && f.pattern.test(sub)) {
        hit = true;
        break;
      }
    }
...
Filter.load = function() {
  var i, w, f, rawFilters, rawPattern, fid, regexEscape, regexType, wordSepS, wordSepE, words, inner, regexWildcard, replaceWildcard;
  this.activeFilters = [];
  if (!(rawFilters = localStorage.getItem('4chan-filters'))) {
    return;
  }
  rawFilters = JSON.parse(rawFilters);
  regexEscape = new RegExp('(\\' + ['/', '.', '*', '+', '?', '(', ')', '[', ']', '{', '}', '\\', '^', '$'].join('|\\') + ')', 'g');
  regexType = /^\/(.*)\/(i?)$/;
  wordSepS = '(?=.*\\b';
  wordSepE = '\\b)';
  regexWildcard = /\\\*/g;
  ...
  try {
      ...
      if (f.active && f.pattern != '') {
        ...
        if (!f.type || f.type == 1 || f.type == 4) {
          ...
          for (i = 0, j = words.length; i < j; ++i) {
            inner = words[i].replace(regexEscape, '\\$1').replace(regexWildcard, replaceWildcard);
    ...
    alert('There was an error processing one of the filters: ' + e + ' in: ' + rawPattern);
  }
};
...
<span><img alt="Close" title="Close" class="pointer" data-cmd="filters-help-close" src="' + Main.icons.cross + '"></span></div>\
...
<span><img alt="Help" class="pointer" title="Help" data-cmd="filters-help-open" src="' + Main.icons.help + '"><img alt="Close" title="Close" class="pointer" data-cmd="filters-close" src="' + Main.icons.cross + '"></span></div>\
<table><thead><tr>\
<th>Order</th>\
<th>On</th>\
<th>Pattern</th>\
...
Filter.buildEntry = function(filter, id) {
  ...
  html += '<td><input type="checkbox"' + (filter.active ? ' checked="checked"></td>' : '></td>');
  html += '<td><input class="fPattern" type="text" value="' + filter.pattern.replace(/"/g, '&quot;') + '"></td>';
  sel = ['', '', '', '', '', ''];
  sel[filter.type] = ' selected="selected"';
  html += '<td><select size="1"><option value="0"' + sel[0] + '>Tripcode</option><option value="1"' + sel[1] + '>Name</option><option value="2"' + sel[2] + '>Comment</option><option value="3"' + sel[3] + '>E-mail</option><option value="4"' + sel[4] + '>ID</option><option value="5"' + sel[5] + '>Subject</option></select></td>';
  ...
  html += '</span></td>';
  html += '<td><input type="checkbox"' + (filter.hide ? ' checked="checked"></td>' : '></td>');
...
Filter.buildPalette = function(id) {
  ...
  for (i = 0; i < rowCount; ++i) {
    ...
    for (j = 0; j < colCount; ++j) {
      html += '<td><div data-cmd="palette-pick" class="colorbox" style="background:' + colors[i][j] + '"></div></td>';
    }
    html += '</tr>'
  }
...
Filter.openPalette = function(target) {
  ...
  picker.style.cssText = 'top:' + pos.top + 'px;left:' + (pos.left - picker.clientWidth - 10) + 'px;';
};
...
SWFEmbed.embedIndex = function(e) {
  var el, cnt, header, icon, backdrop, width, height, cntWidth, cntHeight, maxWidth, maxHeight, docWidth, docHeight, margins, headerHeight, fileName;
  e.preventDefault();
  margins = 10;
  headerHeight = 20;
...
Media.toggleSoundCloud = function(node) {
  ...
  if (node.textContent == 'Remove') {
    ...
    xhr = new XMLHttpRequest();
    xhr.open('GET', '//soundcloud.com/oembed?show_artwork=false&' + 'maxwidth=500px&show_comments=false&format=json&url=' + 'http://' + url);
    xhr.onload = function() {
      var el;
...
Media.toggleVocaroo = function(node) {
  ...
  if (node.textContent == 'Remove') {
    ...
    if (vid && (vid = vid[0].split('/').pop())) {
      ...
      el = document.createElement('div');
      el.className = 'media-embed';
      el.innerHTML = '<embed width="220" height="140" class="media-embed" ' + 'src="//vocaroo.com/mediafoo.swf?playMediaID=' + vid + '&autoplay=0">';
      node.parentNode.insertBefore(el, node.nextElementSibling);
      node.textContent = 'Remove';
    } else {
      node.textContent = 'Error';
...
<span><img alt="Close" title="Close" class="pointer" data-cmd="css-close" src="' + Main.icons.cross + '"></span></div>\
<textarea id="customCSSBox">' + (localStorage.getItem('4chan-css') || '') + '</textarea>\
...
<span><img data-cmd="keybinds-close" class="pointer" alt="Close" title="Close" src="' + Main.icons.cross + '"></span></div>\
<ul>\
...
UA.getSelection = function(raw) {
  var sel;
  if (UA.isOpera && typeof(sel = document.getSelection()) == 'string') {} else {
...
<span><img data-cmd="export-close" class="pointer" alt="Close" title="Close" src="' + Main.icons.cross + '"></span></div>\
...
<p class="center">[<a target="_blank" href="' + str + '">Restore 4chan Settings</a>]</p>';
  document.body.appendChild(cnt);
...
Main.init = function() {
  ...
  if (Main.stylesheet = Main.getCookie(style_group)) {
    ...
    Main.stylesheet = style_group == 'nws_style' ? 'yotsuba_new' : 'yotsuba_b_new';
  }
...
Main.checkMobileLayout = function() {
  ...
  if (window.matchMedia) {
    return window.matchMedia('(max-width: 480px)').matches && localStorage.getItem('4chan_never_show_mobile') != 'true';
  }
...
Main.setStickyNav = function() {
  ...
  hdr.innerHTML = '<img class="pointer" src="' + Main.icons.up + '" data-cmd="totop" alt="▲" title="Top">' + '<img class="pointer" src="' + Main.icons.down + '" data-cmd="tobottom" alt="▼" title="Bottom">';
  Draggable.set(hdr);
  cnt.appendChild(hdr);
  document.body.appendChild(cnt);
};
Main.setTitle = function() {
  ...
  if (!(title = $.cls('subject', $.id('pi' + Main.tid))[0].textContent)) {
    ...
    } else if (title = $.id('fT' + Main.tid)) {
      title = title.lastElementChild.innerHTML;
    } else {
      title = 'No.' + Main.tid;
    }
  }
...
Main.onclick = function(e) {
  ...
  if (cmd = t.getAttribute('data-cmd')) {
    ...
    if (QR.enabled && t.title == 'Quote this post') {
      ...
      if ($.hasClass(t, 'quotelink') && (cmd = t.getAttribute('href').match(QuotePreview.regex)) && cmd[1] != 'rs') {
        e.preventDefault();
      }
    }
  }
...
Main.onThreadMouseOver = function(e) {
  var t = e.target;
  if (Config.quotePreview && $.hasClass(t, 'quotelink') && !$.hasClass(t, 'deadlink')) {
    QuotePreview.resolve(e.target);
  } else if (Config.imageHover && t.hasAttribute('data-md5') && !$.hasClass(t.parentNode, 'deleted')) {
    ImageHover.show(t);
  } else if (Config.filter && t.hasAttribute('data-filtered')) {
    QuotePreview.show(t, t.href ? t.parentNode.parentNode.parentNode : t.parentNode.parentNode);
  }
};
