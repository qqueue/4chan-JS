...
var Parser = {
  ...
  QR = {
    ...
    quotePost: function(a, b) {
      !QR.noCooldown && (Main.threadClosed || !Main.tid &&
        Main.isThreadClosed(a)) ? alert("This thread is closed") : (QR.show(a), QR.addQuote(b))
    },
    addQuote: function(a) {
      var b, c, d;
    ...
    show: function(a) {
      ...
      if (QR.currentTid) Main.tid || QR.currentTid == a || ($.id("qrTid").textContent = $.id("qrResto").value = QR.currentTid = a, $.byName("com")[1].value = "", QR.startCooldown()), Main.hasMobileLayout && ($.id("quickReply").style.top = window.pageYOffset + 25 + "px");
        ...
        Main.hasMobileLayout ? d.style.top = window.pageYOffset + 28 + "px" : Config["QR-position"] ?
          d.style.cssText = Config["QR-position"] : (d.style.right = "0px", d.style.top = "10%");
        ...
        for (c = h.length - 1; b < c; ++b) {
          ...
          else if (g.innerHTML = h[b].children[1].innerHTML, l = "hidden" == g.firstChild.type ? g.lastChild.previousSibling :
            g.firstChild, l.tabIndex = "", "INPUT" == l.nodeName || "TEXTAREA" == l.nodeName) {
            if ("name" == l.name) {
    ...
    onPassError: function() {
      var a, b;
      QR.captchaWidgetCnt || (window.passEnabled = QR.noCaptcha = !1, a = document.createElement("div"), a.id = "qrCaptchaContainer", b = $.id("qrForm"),
        b.insertBefore(a, b.lastElementChild), QR.captchaWidgetCnt = a, QR.renderCaptcha())
    },
    ...
    onKeyDown: function(a) {
      ...
      QR.lenCheckTimeout = setTimeout(QR.checkComLength,
        500)
    },
    ...
    close: function() {
      ...
      (a = $.id("qrFile")) && a.removeEventListener("change",
        QR.startCooldown, !1);
    ...
    submit: function(a) {
      QR.hidePostError();
      QR.presubmitChecks(a) &&
        (QR.auto = !1, QR.xhr = new XMLHttpRequest, QR.xhr.open("POST", document.forms.qrPost.action, !0), QR.xhr.withCredentials = !0, QR.xhr.upload.onprogress = function(a) {
    ...
    presubmitChecks: function(a) {
      return QR.xhr ? (QR.xhr.abort(), QR.xhr = null, QR.showPostError("Aborted."),
        QR.btn.value = "Post", !1) : !a && QR.cooldown ? ((QR.auto = !QR.auto) ? QR.btn.value = QR.cooldown + "s (auto)" : QR.btn.value = QR.cooldown + "s", !1) : !0
    },
  ...
  ThreadHiding = {
    ...
    clear: function(a) {
      ...
      a ? localStorage.removeItem(c) : b ? (a = "This will unhide " + b + " thread" + (1 < b ? "s" : "") + " on /" + Main.board + "/", confirm(a) &&
        localStorage.removeItem(c)) : alert("You don't have any hidden threads on /" + Main.board + "/")
    },
    isHidden: function(a) {
      a = $.id("sa" + a);
    ...
    show: function(a) {
      ...
      Main.hasMobileLayout ? (b.textContent = "Hide", $.removeClass(b, "mobile-tu-show"), $.cls("postLink", c)[0].appendChild(b), c.style.display = null, $.removeClass(c.nextElementSibling,
        "mobile-hr-hidden")) : (b.firstChild.src = Main.icons.minus, $.removeClass(c, "post-hidden"));
  ...
  ReplyHiding = {
    ...
    toggleR: function(a) {
      var b;
      b =
        this.isHidden(a) ? this.show : this.hide;
  ...
  ThreadWatcher = {
    init: function() {
      ...
      this.isRefreshing = !1;
      Main.hasMobileLayout &&
        (c = document.createElement("a"), c.href = "#", c.textContent = "TW", c.addEventListener("click", ThreadWatcher.toggleList, !1), a = $.id("settingsWindowLinkMobile"), a.parentNode.insertBefore(c, a), a.parentNode.insertBefore(document.createTextNode(" "), a));
      if (location.hash && (b = location.hash.split("lr")[1])) {
        if (b = $.id("pc" + b)) b.nextElementSibling && (b = b.nextElementSibling, (c = $.id("p" + b.id.slice(2))) && $.addClass(c, "highlight")), a = b.getBoundingClientRect(), (0 > a.top || a.bottom > document.documentElement.clientHeight) &&
      ...
      a.innerHTML = '<div class="drag" id="twHeader">' + (Main.hasMobileLayout ?
        '<img id="twClose" class="pointer" src="' + Main.icons.cross + '" alt="X">' : "") + "Thread Watcher" + (UA.hasCORS ? '<img id="twPrune" class="pointer right" src="' + Main.icons.refresh + '" alt="R" title="Refresh"></div>' : "</div>");
    ...
    syncStorage: function(a) {
      var b;
      a.key && (b = a.key.split("-"), "4chan" != b[0] || "watch" != b[1] || b[2] || a.newValue == a.oldValue ||
        (ThreadWatcher.load(), ThreadWatcher.build(!0)))
    },
    load: function() {
    ...
    build: function(a) {
      var b, c, d, e;
      b = "";
      for (d in this.watched) c = d.split("-"), b += '<li id="watch-' + d + '"><span class="pointer" data-cmd="unwatch" data-id="' + c[0] + '" data-board="' + c[1] + '">&times;</span> <a href="' + Main.linkToThread(c[0], c[1]) + "#lr" + this.watched[d][1] + '"', -1 == this.watched[d][1] ?
        b += ' class="deadlink">' : (e = this.watched[d][3] ? "archivelink" : !1, b = this.watched[d][2] ? b + (' class="' + (e ? e + " " : "") + 'hasNewReplies">(' + this.watched[d][2] + ") ") : b + ((e ? 'class="' + e + '"' : "") + ">")), b += "/" + c[1] + "/ - " + this.watched[d][0] + "</a></li>";
    ...
    rebuildButtons: function() {
      var a, b, c;
      b = $.cls("wbtn");
      for (a = 0; btn = b[a]; ++a) c = btn.getAttribute("data-id") + "-" + Main.board, ThreadWatcher.watched[c] ? btn.hasAttribute("data-active") || (btn.src = Main.icons.watched,
        btn.setAttribute("data-active", "1")) : btn.hasAttribute("data-active") && (btn.src = Main.icons.notwatched, btn.removeAttribute("data-active"))
    },
    ...
    refreshWithAutoWatch: function() {
      ...
      if (Config.filter) {
        ...
        c ? (a = $.id("twPrune"), a.src = Main.icons.rotate,
          this.isRefreshing = !0, this.fetchCatalogs(e, c)) : this.refresh()
      } else this.refresh()
    },
    ...
    refresh: function() {
      ...
      if (d = $.id("watchList").children.length)
        for (c in a = b = 0, e = $.id("twPrune"), e.src = Main.icons.rotate, ThreadWatcher.isRefreshing = !0, ThreadWatcher.setRefreshTimestamp(),
          ThreadWatcher.watched) setTimeout(ThreadWatcher.fetch, b, c, ++a == d ? e : null), b += 200
    },
    ...
    fetch: function(a, b) {
      ...
      if (-1 == ThreadWatcher.watched[a][1]) {
        ...
        if (200 == this.status) {
          ...
          d > ThreadWatcher.watched[a][2] &&
            (ThreadWatcher.watched[a][2] = d);
      ...
      if (b = a.getAttribute("href").match(/^(?:thread\/)([0-9]+)#p([0-9]+)$/)) c = b[1], d = b[2], e = a.parentNode, e.textContent = "Loading...", $.get("//a.4cdn.org/" +
        Main.board + "/thread/" + c + ".json", {
          onload: function() {
            ...
            } else 404 == this.status ? e.textContent = "This thread doesn't exist anymore." :
              (e.textContent = "Connection Error", console.log("ThreadExpansion: " + this.status + " " + this.statusText))
          },
          onerror: function() {
