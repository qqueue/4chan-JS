          ...
          for (e = 0; b = c[e]; ++e)
            if (b.active && "" != b.pattern) {
              ...
              if (b.type && 1 != b.type && 4 != b.type)
                if (match = d.match(h)) pattern = new RegExp(match[1], match[2]);
                else if ('"' == d[0] && '"' == d[d.length - 1]) pattern = new RegExp(d.slice(1, -1).replace(f, "\\$1"));
              else {
                g = d.split(" ");
                pattern = "";
                a = 0;
