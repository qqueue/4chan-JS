...
var Parser = {
    ...
    buildHTMLFromJSON: function(a, b, c, d) {
      ...
      a.filedeleted ? g = '<div id="f' + a.no + '" class="file"><span class="fileThumb"><img src="' + Parser.icons.del + '" class="fileDeletedRes" alt="File deleted."></span></div>' : a.ext && (g = Parser.decodeSpecialChars(a.filename), q = y = a.filename + a.ext, g.length > (f ? 40 : 30) && (q = Parser.encodeSpecialChars(g.slice(0, f ? 35 : 25)) + "(...)" + a.ext, J = !0), a.tn_w || a.tn_h || ".gif" != a.ext || (a.tn_w = a.w, a.tn_h = a.h), p = 1048576 <= a.fsize ? (0 | a.fsize / 1048576 * 100 + .5) / 100 + " M" : 1024 < a.fsize ? (0 | a.fsize / 1024 + .5) + " K" : a.fsize + " ", a.spoiler && !Config.revealSpoilers && (q = "Spoiler Image", l = '" title="' + y + '"', x = " imgspoiler", n = "//s.4cdn.org/image/spoiler" + (Parser.customSpoiler[b] || "") + ".png", a.tn_w = 100, a.tn_h = 100), n || (n = "//0.t.4cdn.org/" + b + "/" + a.tim + "s.jpg"), g = ".pdf" == a.ext ? "PDF" : a.w + "x" + a.h, "f" != b ? (m = m + "/" + a.tim + a.ext, k = '<a class="fileThumb' + x + '" href="' + m + '" target="_blank"><img src="' + n + '" alt="' + p + 'B" data-md5="' + a.md5 + '" style="height: ' + a.tn_h + "px; width: " + a.tn_w + 'px;"><div class="mFileInfo mobile">' + p + "B " + a.ext.slice(1).toUpperCase() + "</div></a>", l = '<div class="fileText" id="fT' + a.no + l + ">File: <a" + (J ? ' title="' + y + '"' : "") + ' href="' + m + '" target="_blank">' + q + "</a> (" + p + "B, " + g + ")</div>") : (m = m + "/" + a.filename + a.ext, g += ", " + a.tag, l = '<div class="fileText" id="fT' + a.no + '">File: <a href="' +
        m + '" target="_blank">' + a.filename + ".swf</a> (" + p + "B, " + g + ")</div>"), g = '<div id="f' + a.no + '" class="file">' + l + k + "</div>");
      ...
      e.id = "pc" + a.no;
      e.innerHTML = (f ? "" : '<div class="sideArrows" id="sa' + a.no + '">&gt;&gt;</div>') + '<div id="p' + a.no + '" class="post ' + C + D + '"><div class="postInfoM mobile" id="pim' +
        a.no + '"><span class="nameBlock' + t + '"><span class="name">' + k + "</span>" + s + r + u + h + z + "<br>" + d + '</span><span class="dateTime postNum" data-utc="' + a.time + '">' + a.now + ' <a href="' + a.no + "#p" + a.no + '" title="Link to this post">No.</a><a href="javascript:quote(\'' + a.no + '\');" title="Reply to this post">' + a.no + "</a></span></div>" + (f ? g : "") + '<div class="postInfo desktop" id="pi' + a.no + '"' + (b != Main.board ? ' data-board="' + b + '"' : "") + '><input type="checkbox" name="' + a.no + '" value="delete"> ' + d + '<span class="nameBlock' +
  ...
  QR = {
    ...
    submitDirect: function(a) {
      ...
      QR.presubmitChecks(a) && (QR.auto = !1, !a && (b = $.id("qrCapField")) && "" == b.value ? (QR.showPostError("You forgot to type in the CAPTCHA."), b.focus()) : (QR.xhr = new XMLHttpRequest, QR.xhr.open("POST", document.forms.qrPost.action, !0), QR.xhr.withCredentials = !0, QR.xhr.upload.onprogress = function(a) {
        ...
        QR.btn.value = "Post";
        if (200 == this.status)
          if (a = this.responseText.match(/"errmsg"[^>]*>(.*?)<\/span/)) QR.reloadCaptcha(!0), QR.showPostError(a[1]);
          else {
