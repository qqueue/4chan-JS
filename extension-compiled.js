...
Parser.init = function() {
  ...
  if (Config.filter || Config.linkify || Config.embedSoundCloud || Config.embedYouTube || Main.hasMobileLayout) {
    this.needMsg = true;
  }
  ...
  if (Config.localTime) {
    if (o = (new Date).getTimezoneOffset()) {
      ...
      this.utcOffset = 'Timezone: UTC' + (o < 0 ? '+' : '-') + h + ((m = a - h * 60) ? (':' + m) : '');
    } else {
...
Parser.buildHTMLFromJSON = function(data, board, standalone, fromQuote) {
  ...
  if (data.resto == 0) {
    ...
    if (standalone) {
      ...
      mobileLink = '<div class="postLink mobile"><span class="info">' + tmp + '</span><a href="' + 'thread/' + data.no + '" class="button">View Thread</a></div>';
      postType = 'op';
  ...
  switch (data.capcode) {
    ...
    case 'manager':
      ...
      break;
    case 'founder':
      capcodeStart = ' <strong class="capcode hand id_admin" ' + 'title="Highlight posts by the Founder">## Founder</strong>';
      capcodeClass = ' capcodeAdmin';

      capcode = ' <img src="' + Parser.icons.founder + '" ' + 'alt="This user is 4chan\'s Founder." ' + 'title="This user is 4chan\'s Founder." class="identityIcon">';
      break;
  }
  ...
  if (data.filedeleted) {
    ...
    if (board != 'f') {
      ...
      imgSrc = '<a class="fileThumb' + fileClass + '" href="' + filePath + '" target="_blank"><img src="' + fileThumb + '" alt="' + size + 'B" data-md5="' + data.md5 + '" style="height: ' + data.tn_h + 'px; width: ' + data.tn_w + 'px;">' + '<div data-tip data-tip-cb="mShowFull" class="mFileInfo mobile">' + size + 'B ' + data.ext.slice(1).toUpperCase() + '</div></a>';
  ...
  if (name.length > 30) {
    mName = '<span class="name" data-tip data-tip-cb="mShowFull">' + Parser.truncate(name, 30) + '(...)</span> ';
  } else {
  ...
  if (isOP) {
    ...
    if (data.closed) {
      if (data.archived) {
        threadIcons += '<img class="archivedIcon retina" title="Archived" alt="Archived" src="' + Main.icons2.archived + '"> ';
      } else {
    ...
    if (data.sub === undefined) {
      ...
      subject = '<span class="subject" data-tip data-tip-cb="mShowFull">' + Parser.truncate(data.sub, 30) + '(...)</span> ';
    } else {
  ...
  container.innerHTML =
    ...
    quoteLink + '" title="Reply to this post">' + data.no + '</a> ' + threadIcons + replySpan +
    '</span>' +
    '</div>' +
    (isOP ? '' : fileHtml) +
    '<blockquote class="postMessage" id="m' + data.no + '">' + (data.com || '') + capcode_replies + summary + '</blockquote> ' +
    '</div>' + mobileLink;
...
Parser.parsePost = function(pid, tid) {
  ...
  if (Config.revealSpoilers && (file = document.getElementById('f' + pid)) && (file = file.children[1])) {
    if ($.hasClass(file, 'imgspoiler')) {
      ...
      img.src = '//i.4cdn.org' + (file.pathname.replace(/([0-9]+).+$/, '/$1s.jpg'));
  ...
  if (Config.localTime) {
    if (hasMobileLayout) {
      ...
      el.firstChild.nodeValue = Parser.getLocaleDate(new Date(el.getAttribute('data-utc') * 1000)) + ' ';
    } else {
...
Parser.getLocaleDate = function(date) {
  return ('0' + (1 + date.getMonth())).slice(-2) + '/' + ('0' + date.getDate()).slice(-2) + '/' + ('0' + date.getFullYear()).slice(-2) + '(' + this.weekdays[date.getDay()] + ')' + ('0' + date.getHours()).slice(-2) + ':' + ('0' + date.getMinutes()).slice(-2) + ':' + ('0' + date.getSeconds()).slice(-2);
};
...
Parser.parseBacklinks = function(pid, tid) {
  ...
  for (i = 0; j = backlinks[i]; ++i) {
    ...
    if (!Main.hasMobileLayout) {
      ...
      bl.innerHTML = '<a href="' + href + '" class="quotelink">&gt;&gt;' + pid + '</a><a href="' + href + '" class="quoteLink"> #</a> ';
    }
...
Parser.buildSummary = function(tid, oRep, oImg) {
  ...
  el.innerHTML = oRep + oImg + ' omitted. <a href="thread/' + tid + '" class="replylink">Click here</a> to view.';
...
PostMenu.open = function(btn) {
  ...
  html = '<ul><li data-cmd="report" data-id="' + pid + (board ? ('" data-board="' + board + '"') : '"') + '">Report post</li>';
...
QuotePreview.show = function(link, post, remote) {
  ...
  if (remote) {
    ...
    post.className += ' preview' + (!$.hasClass(link.parentNode.parentNode, 'backlink') ? ' reveal-spoilers' : '');
...
ImageHover.showWebMDuration = function(el, thumb) {
  ...
  if (el.mozHasAudio === true || el.webkitAudioDecodedByteCount > 0 || (el.audioTracks && el.audioTracks.length)) {
    sound = ' (audio)';
  } else {
    sound = '';
  }
...
QR.show = function(tid) {
  ...
  cnt.innerHTML =
    '<div id="qrHeader" class="drag postblock">Reply to Thread No.<span id="qrTid">' + tid + '</span><img alt="X" src="' + Main.icons.cross + '" id="qrClose" ' + 'class="extButton" title="Close Window"></div>';

  form = postForm.parentNode.cloneNode(false);
  form.setAttribute('name', 'qrPost');
  form.innerHTML =
    '<input type="hidden" value="' + $.byName('MAX_FILE_SIZE')[0].value + '" name="MAX_FILE_SIZE">' + '<input type="hidden" value="regist" name="mode">' + '<input id="qrResto" type="hidden" value="' + tid + '" name="resto">';
...
ThreadWatcher.init = function() {
  ...
  cnt.innerHTML = '<div class="drag" id="twHeader">' + (Main.hasMobileLayout ? ('<img id="twClose" class="pointer" src="' + Main.icons.cross + '" alt="X">') : '') + 'Thread Watcher' + (UA.hasCORS ? ('<img id="twPrune" class="pointer right" src="' + Main.icons.refresh + '" alt="R" title="Refresh"></div>') : '</div>');
...
ThreadUpdater.onScroll = function(e) {
  if (ThreadUpdater.hadAuto &&
    (document.documentElement.scrollHeight <= (window.innerHeight + window.pageYOffset) && !document[ThreadUpdater.hidden])) {
...
ThreadUpdater.onload = function() {
  ...
  if (this.status == 200) {
    ...
    if (count) {
      ...
      autoscroll = (
        Config.autoScroll && document[self.hidden] && doc.scrollHeight == (window.innerHeight + window.pageYOffset)
      );
...
Filter.match = function(post, board) {
  ...
  for (i = 0; f = filters[i]; ++i) {
    ...
    // comment
      ...
      if (com === undefined) {
        this.entities.innerHTML = post.com.replace(/<br>/g, '\n').replace(/[<[^>]+>/g, '');
...
Filter.exec = function(cnt, pi, msg, tid) {
  ...
  for (i = 0; f = filters[i]; ++i) {
    ...
    // tripcode
    if (f.type == 0) {
      if ((trip !== undefined || (trip = pi.getElementsByClassName('postertrip')[0])) && f.pattern == trip.textContent) {
        hit = true;
        break;
      }
    }
  ...
  if (hit) {
    if (f.hide) {
      ...
      if (!tid) {
        ...
        el.innerHTML = '[<a data-cmd="unfilter" data-filtered="1" href="thread/' + tid + '">View</a>]';
      }
...
Filter.buildEntry = function(filter, id) {
  ...
  // Boards
  html += '<td><input class="fBoards" type="text" value="' + (filter.boards !== undefined ? filter.boards : '') + '"></td>';
  ...
  html += '<td><select size="1"><option value="0"' + sel[0] + '>Tripcode</option><option value="1"' + sel[1] + '>Name</option><option value="2"' + sel[2] + '>Comment</option><option value="4"' + sel[4] + '>ID</option><option value="5"' + sel[5] + '>Subject</option><option value="6"' + sel[6] + '>Filename</option></select></td>';
  ...
  // Auto
  html += '<td><input type="checkbox"' + (filter.auto ? ' checked="checked"></td>' : '></td>');
...
var Linkify = {
  ...
  funk: function(match, pfx, url, o, str) {
    ...
    return pfx + '<a href="' + Linkify.derefer + encodeURIComponent(url.replace(/\u200b/g, '')) + '" target="_blank" class="linkified" rel="nofollow">' + url + '</a>' + sfx;
  }
};
...
Media.toggleSoundCloud = function(node) {
  ...
  if (node.textContent == 'Remove') {
    ...
    xhr = new XMLHttpRequest();
    xhr.open('GET', '//soundcloud.com/oembed?show_artwork=false&' + 'maxwidth=500px&show_comments=false&format=json&url=' + 'http://' + url.replace(/^https?:\/\//i, ''));
    xhr.onload = function() {
      var el;
...
Media.replaceYouTube = function(link, o, str) {
  ...
  return pfx + ' [<a href="javascript:;" data-cmd="embed" data-type="yt">' + (!Main.hasMobileLayout ? 'Embed' : 'Open') + '</a>]';
};
...
Main.checkMobileLayout = function() {
  ...
  if (window.matchMedia) {
    return (window.matchMedia('(max-width: 480px)').matches || window.matchMedia('(max-device-width: 480px)').matches) && localStorage.getItem('4chan_never_show_mobile') != 'true';
  }
...
Main.onclick = function(e) {
  ...
  if (cmd = t.getAttribute('data-cmd')) {
    ...
    } else if (Config.imageExpansion && e.which == 1 && t.parentNode && $.hasClass(t.parentNode, 'fileThumb') && t.parentNode.nodeName == 'A' && !$.hasClass(t.parentNode, 'deleted') && !$.hasClass(t, 'mFileInfo')) {
    ...
    } else if (Main.isMobileDevice && Config.quotePreview && $.hasClass(t, 'quotelink') && t.getAttribute('href').match(QuotePreview.regex)) {
      e.preventDefault();
...
Main.onThreadMouseOut = function(e) {
  ...
  if (Config.quotePreview && $.hasClass(t, 'quotelink')) {
    ...
    (t.hasAttribute('data-md5') || (t.href && !$.hasClass(t.parentNode, 'fileText') && /i\.4cdn\.org\/[a-z0-9]+\/[0-9]+\.(gif|jpg|png|webm)$/.test(t.href)))
  ) {
    ImageHover.hide();
  } else if ($.hasClass(t, 'dateTime')) {
