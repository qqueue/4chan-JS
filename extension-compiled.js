    ...
    load: function() {
      ...
      if (d = localStorage.getItem("4chan-filters")) {
        ...
        try {
          for (f = 0; c = d[f]; ++f)
            if (c.active && "" != c.pattern) {
              ...
              if (c.type && 1 != c.type && 4 != c.type)
                ...
                pattern = "";
                a = 0;
                for (b = k.length; a < b; ++a) l = k[a].replace(h, "\\$1").replace(m, "[^\\s]*"), pattern += "(?=.*\\b" + l + "\\b)";
          ...
          alert("There was an error processing one of the filters: " + n + " in: " + e)
        }
      }
    },
